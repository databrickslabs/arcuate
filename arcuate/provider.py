...
def normalize_experiment_df(experiment_infos_df: DataFrame) -> DataFrame:
    ...
    return experiment_infos_df.select(
...
def pickle_df(experiment_infos_df: DataFrame) -> DataFrame:
    return experiment_infos_df.withColumn(
        "model_payload", pickle_run_model_udf("run_info.run_id")
...
def export_experiments(
    ...
    normalized = pickle_df(normalize_experiment_df(experiment_infos_df))
...
def pickle_model(model_info_df: DataFrame, model_name: str) -> DataFrame:
    return (
...
def export_models(
    ...
    model_info_df = pickle_model(model_info_df, model_name)
